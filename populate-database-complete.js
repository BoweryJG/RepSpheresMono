...
// Function to map procedure data to match our schema
function mapProcedureData(procedure) {
    const {
        futureOutlook,
        marketSize2025,
        primaryAgeGroup,
    ...
    // Map fields to match database schema (camelCase to snake_case)
    if (futureOutlook !== undefined) mapped.future_outlook = futureOutlook;
    if (marketSize2025 !== undefined) mapped.market_size_2025 = marketSize2025;
    if (primaryAgeGroup !== undefined) mapped.primary_age_group = primaryAgeGroup;
    
    return {
        procedureData: mapped,
        companies
    };
}
...
async function populateDatabase() {
    try {
        ...
        // Combine all procedures and map the data
        const allProcedures = [
            ...dentalProcedures.map(mapProcedureData),
            ...aestheticProcedures.map(mapProcedureData)
        ];
        
        // Extract unique companies
        const companies = new Set();
        allProcedures.forEach(({ companies: procCompanies }) => {
            if (procCompanies && Array.isArray(procCompanies)) {
                procCompanies.forEach(company => {
        ...
        for (const { procedureData, companies: procCompanies } of allProcedures) {
            try {
                console.log(`Processing procedure: ${procedureData.name}`);
                
                // Check if procedure already exists
                const { data: existingProcedure, error: fetchError } = await supabase
                    .from('procedures')
                    .select('id')
                    .eq('name', procedureData.name)
                    .eq('category', procedureData.category)
                    .single();
                
                let procedureId;
                
                if (existingProcedure) {
                    console.log(`Procedure ${procedureData.name} (${procedureData.category}) already exists, updating...`);
                ...
                if (procCompanies && Array.isArray(procCompanies)) {
                    ...
                    if (relationships.length > 0) {
                        console.log(`Linking ${relationships.length} companies to procedure ${procedureData.name}`);
                        ...
                        if (deleteError && deleteError.code !== '42P01') { // Ignore "relation does not exist" error
                ...
                console.error(`Error processing procedure ${procedureData.name}:`, error);
